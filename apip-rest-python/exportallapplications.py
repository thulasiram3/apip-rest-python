...
def main(*mainargs):
    ...
    if len(mainargs) == 0 : #if len(sys.argv)>1 and len(sys.argv)<5:
        parser = argparse.ArgumentParser(description='This program exports all Applications from APICS and persist them on the file system') 
        ...
        parser.add_argument('--destdir', dest='destdir', default='./', help='directory path on local file system where all the exported Apps and artifacts will be saved to; defaults to current directory', required=False )
    ...
    #apisvr.getallapps(destdir)
    getallapps = apisvr.getall('application') #get the real method
    getallapps(destdir) # and execute it
    return 'success'
